...
class Tinder:
    ...
    def dashboard(self):
        ...
        Button(menu, text='View Matches', width=13, command=lambda: self.view_matches()).pack(side=LEFT, expand=True,
                                                                                              fill=X)
    ...
    def view_all_user(self, i=0):
        ...
        self.mycursor.execute(
            """SELECT `user_id`,`name`,`gender`,`age`,`city` FROM `users` WHERE `user_id` NOT LIKE '{}' AND `user_id` NOT IN(SELECT `juliet_id` FROM `proposal` WHERE `romeo_id`='{}')""".format(
    ...
    def propose(self, juliet_id, next_user_id):
        self.auth()
        self.mycursor.execute(
            """INSERT INTO `proposal` SET `romeo_id`='{}',`juliet_id`='{}'""".format(self.current_user_id, juliet_id))
        self.conn.commit()
    ...
    def view_proposal(self):
        self.auth()
        self.mycursor.execute(
            """SELECT u.`name`,u.`gender`,u.`city`,u.`age` FROM `proposal` p JOIN `users` u ON p.`juliet_id` = u.`user_id` WHERE p.`romeo_id` LIKE '{}'""".format(
    ...
    def view_requests(self):
        self.auth()
        self.mycursor.execute(
            """SELECT u.`name`,u.`gender`,u.`city`,u.`age` FROM `proposal` p JOIN `users` u ON p.`romeo_id` = u.`user_id` WHERE p.`juliet_id` LIKE '{}'""".format(
                self.current_user_id))
    ...
    def view_matches(self):
        self.auth()
        self.mycursor.execute(
            """SELECT `name`,`gender`,`age`,`city` FROM `users` WHERE `user_id` IN (SELECT `juliet_id` FROM `proposal` WHERE `romeo_id` LIKE '{}' AND `juliet_id` IN (SELECT `romeo_id` FROM `proposal` WHERE `juliet_id` LIKE '{}'))""".format(
                self.current_user_id, self.current_user_id))
    ...
    def login_form(self):
        ...
        Button(self.root, text='Login', width='15', command=lambda: self.login()).grid(row=3, column=0, columnspan=2,
                                                                                       padx=10, pady=10)
        Button(self.root, text='Register', width='15', command=lambda: self.registration_form()).grid(row=4, column=0,
                                                                                                      columnspan=2,
                                                                                                      padx=10, pady=10)
